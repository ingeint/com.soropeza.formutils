...
public class WGrid extends Grid implements ValueChangeListener, EventListener<Event> {
	...
	public Object getValue(int row, int column) {
		...
		if (component instanceof Combobox) {
			...
			value = attributeColumn.getContext() != null ? Integer.parseInt(attributeColumn.getContext()) : 0;
		}
		return value;
	}
	...
	public void onEvent(Event evt) throws Exception {
		...
		if (evt.getTarget() instanceof Decimalbox) {
			...
			int iColumn = getColumnIndex(cell, row);
			if (listener != null) {
				GridValueChangeEvent event = new GridValueChangeEvent(source, iRow, iColumn, numberColumn.getOldValue(),
						source.getValue());
				listener.gridValueChange(event);
			}
